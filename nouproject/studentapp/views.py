    ...
    try:
        if request.session['rollno']!=None:
            ...
            if request.method=="POST":
                ...
                Ques=Question(question=question, postedby=postedby, postdate=posteddate)
                Ques.save()
...
def postans(request):
    try:
        if request.session['rollno']!=None:
            ...
            answeredby=stu.name
            postdate=date.today()
            ans=Answer(qid=qid,answer=answer,answredby=answeredby,postedate=postdate)
            ans.save()
            return redirect('studentapp:postquestion')
            return render(request,"postans.html",{'stu':stu})
    except KeyError:
        return redirect('nouapp:login')
                ...
                if oldpassword==presentpassword.password:
                    if oldpassword!=newpassword:
                        if newpassword==confirmpassword:
                            ...
                            messages.info(request,'Newpassword and confirmpassword must be save')
                    else:
                        messages.info(request,'Newpassword and confirmpassword must not be save')
                else:
                    messages.info(request,'oldpassword is incorrect')
